...
const WallEarthdayPixel = require('js/wall_earthday/components/WallEarthdayPixel');

const hexGrid = require('js/wallow/WallowHexGrid');

const Y_AXIS_SCALE = Math.sqrt(3) / 2;

const WallEarthdayHex = React.createClass({
    render() {
        ...
        return (
            <g style={{transform: `translate(${tx}px, ${ty}px)`}}>
                {shapes.map((polygon, index) => {
                    return <WallEarthdayPixel key={index} tx={tx} ty={ty} polygon={polygon} />;
...
module.exports = WallEarthdayHex;
