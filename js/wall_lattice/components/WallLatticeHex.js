        ...
        return (
            <g style={{transform: `translate(${tx}px, ${ty}px) scale(${cell.scale}) rotate(${cell.rotation}deg)`}}>
                {SHOW_SHAPES && shapes.map((polygon, index) => {
                    return <WallLatticePixel type="polygon" key={index} tx={tx} ty={ty} polygonOrEdge={polygon} />;
                ...
                {SHOW_EDGES && edges.map((edge, index) => {
                    return <WallLatticePixel type="edge" key={index} tx={tx} ty={ty} polygonOrEdge={edge} />;
                })}
            </g>
        );
    },
